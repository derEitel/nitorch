...
class syntheticDataset(Dataset):
    ...
    def __init__(self):
        ...
        data_class1 = np.random.multivariate_normal(
            mean1, covariance1, n_samples1
        )

        n_samples2 = 750
        mean2 = np.array([-5, 5, -5])
        covariance2 = np.array([[10, 0, 0], [0, 10, 0], [0, 0, 10]])
        data_class2 = np.random.multivariate_normal(
            mean2, covariance2, n_samples2
        )
...
if __name__ == "__main__":
    ...
    assert (
        slope < 0
    ), "Test 1-a Failed - The loss is not reducing with training."
