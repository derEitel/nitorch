...
def get_biobank(path,label_col, bs=8, test_size=0.2, mri_type='T2',  z_factor=None, num_samples=None, labels_to_keep=None,
             transforms=default_transforms, random_state=None, balance=False, **kwargs):
    mri_type = mri_type.upper()
    assert mri_type in ['T1', 'T2'], "Argument mri_type has to be one of T1 or T2"

    db = DataBunch(source_dir="/analysis/ritter/data/UKbiobank/UKBiobank_BIDS",
                   table=f'../tables/participants_transMRI_{mri_type}.csv',
                     path=path,
                     mask=None,
                     label_col=label_col,
                     labels_to_keep=labels_to_keep, random_state=random_state, balance=balance,
                     test_size=test_size, num_samples=num_samples, z_factor=z_factor,
                     transforms=transforms, **kwargs)
    db.build_dataloaders(bs=bs)
    return db