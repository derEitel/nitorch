...
class MRIDataset(Dataset):
    ...
    def __init__(
        ...
        self.class_weights = np.array(list(self.label_counts.values())) / len(
            labels
        )
...
class DataBunch:
    ...
    def __init__(
            ...
            if os.path.exists(self.path / self.CACHE_NAME):
                ...
                if ans == "y":
                    try:
                        ...
                        logger.warning(
                            "Pickled DataBunch is corrupted at {}".format(
                                self.path
                            )
                        )
                        print(
        ...
        self.table = table
        self.image_dir = (
            self.source_dir / image_dir if image_dir is not None else None
        )
        self.z_factor = z_factor
        ...
        self.mask = (
            load_nifti(
                str(mask), z_factor=z_factor, force_to_shape=self.force_to_shape
            )
            if mask is not None
            else None
        )
        ...
        get_file_path = (
            get_image_filepath if get_file_path is None else get_file_path
        )
        ...
        self.label2id = {
            k: v for k, v in zip(self.classes, np.arange(len(self.classes)))
        }
    ...
    def build_datasets(
        ...
        self.train_ds = MRIDataset(
            ...
            [self.label2id[l] for l in df_trn[self.LABEL]],
        ...
        self.test_ds = MRIDataset(
            df_tst[self.FILE].tolist(),
            [self.label2id[l] for l in df_tst[self.LABEL]],
            id2label=self.id2label,
            z_factor=self.z_factor,
            transform=transforms,
            force_to_shape=self.force_to_shape,
    ...
    def normalize(self, use_samples: int = None):
        ...
        self.test_ds.mean, self.test_ds.std = (
            self.train_ds.mean,
            self.train_ds.std,
        )
    ...
    def print_stats(self):
        ...
        def get_stats(df):
            image_count, patient_count = (
                [len(df)],
                [len(df[self.PTID].unique())],
            )
            image_count += [
                len(df[df[self.LABEL] == cls]) for cls in self.classes
            ]
            patient_count += [
                len(df[df[self.LABEL] == cls][self.PTID].unique())
                for cls in self.classes
            ]
            return image_count + patient_count

        stats = [
            ["Train"] + get_stats(self.df_trn),
